...
use crate::value::{
    ArithmeticOps, ExtendInto, Float, Integer, LittleEndianConvert,
...
impl RuntimeValueInternal {
    pub fn with_type(self, ty: ValueType) -> RuntimeValue {
        match ty {
            ValueType::I32 => {
                RuntimeValue::I32(<_>::from_runtime_value_internal(self))
            }
            ValueType::I64 => {
                RuntimeValue::I64(<_>::from_runtime_value_internal(self))
            }
            ValueType::F32 => {
                RuntimeValue::F32(<_>::from_runtime_value_internal(self))
            }
            ValueType::F64 => {
                RuntimeValue::F64(<_>::from_runtime_value_internal(self))
            }
        }
    }
}
...
impl Interpreter {
    pub fn new(
        ...
        let mut value_stack =
            StackRecycler::recreate_value_stack(&mut stack_recycler);
        for &arg in args {
            let arg = arg.into();
        ...
        let mut call_stack =
            StackRecycler::recreate_call_stack(&mut stack_recycler);
                ...
                RunResult::NestedCall(nested_func) => {
                    ...
                    match *nested_func.as_internal() {
                        FuncInstanceInternal::Internal { .. } => {
                            let nested_context =
                                FunctionContext::new(nested_func.clone());
                        ...
                        FuncInstanceInternal::Host {
                            ...
                            let args = prepare_function_args(
                                signature,
    ...
    fn run_instruction(
        ...
        match instruction {
            Unreachable => self.run_unreachable(fcx),

            Br(target) => self.run_br(fcx, *target),
            BrIfEqz(target) => self.run_br_eqz(*target),
            BrIfNez(target) => self.run_br_nez(*target),
            BrTable(targets) => self.run_br_table(*targets),
            Return(drop_keep) => self.run_return(*drop_keep),

            Call(index) => self.run_call(fcx, *index),
            CallIndirect(index) => self.run_call_indirect(fcx, *index),

            Drop => self.run_drop(),
            Select => self.run_select(),

            GetLocal(depth) => self.run_get_local(*depth),
            SetLocal(depth) => self.run_set_local(*depth),
            TeeLocal(depth) => self.run_tee_local(*depth),
            GetGlobal(index) => self.run_get_global(fcx, *index),
            SetGlobal(index) => self.run_set_global(fcx, *index),

            I32Load(offset) => self.run_load::<i32>(fcx, *offset),
            I64Load(offset) => self.run_load::<i64>(fcx, *offset),
            F32Load(offset) => self.run_load::<F32>(fcx, *offset),
            F64Load(offset) => self.run_load::<F64>(fcx, *offset),
            I32Load8S(offset) => self.run_load_extend::<i8, i32>(fcx, *offset),
            I32Load8U(offset) => self.run_load_extend::<u8, i32>(fcx, *offset),
            I32Load16S(offset) => {
                self.run_load_extend::<i16, i32>(fcx, *offset)
            }
            I32Load16U(offset) => {
                self.run_load_extend::<u16, i32>(fcx, *offset)
            }
            I64Load8S(offset) => self.run_load_extend::<i8, i64>(fcx, *offset),
            I64Load8U(offset) => self.run_load_extend::<u8, i64>(fcx, *offset),
            I64Load16S(offset) => {
                self.run_load_extend::<i16, i64>(fcx, *offset)
            }
            I64Load16U(offset) => {
                self.run_load_extend::<u16, i64>(fcx, *offset)
            }
            I64Load32S(offset) => {
                self.run_load_extend::<i32, i64>(fcx, *offset)
            }
            I64Load32U(offset) => {
                self.run_load_extend::<u32, i64>(fcx, *offset)
            }

            I32Store(offset) => self.run_store::<i32>(fcx, *offset),
            I64Store(offset) => self.run_store::<i64>(fcx, *offset),
            F32Store(offset) => self.run_store::<F32>(fcx, *offset),
            F64Store(offset) => self.run_store::<F64>(fcx, *offset),
            I32Store8(offset) => self.run_store_wrap::<i32, i8>(fcx, *offset),
            I32Store16(offset) => self.run_store_wrap::<i32, i16>(fcx, *offset),
            I64Store8(offset) => self.run_store_wrap::<i64, i8>(fcx, *offset),
            I64Store16(offset) => self.run_store_wrap::<i64, i16>(fcx, *offset),
            I64Store32(offset) => self.run_store_wrap::<i64, i32>(fcx, *offset),

            CurrentMemory => self.run_current_memory(fcx),
            GrowMemory => self.run_grow_memory(fcx),

            I32Const(val) => self.run_const((*val).into()),
            I64Const(val) => self.run_const((*val).into()),
            F32Const(val) => self.run_const((*val).into()),
            F64Const(val) => self.run_const((*val).into()),

            I32Eqz => self.run_eqz::<i32>(),
            I32Eq => self.run_eq::<i32>(),
            I32Ne => self.run_ne::<i32>(),
            I32LtS => self.run_lt::<i32>(),
            I32LtU => self.run_lt::<u32>(),
            I32GtS => self.run_gt::<i32>(),
            I32GtU => self.run_gt::<u32>(),
            I32LeS => self.run_lte::<i32>(),
            I32LeU => self.run_lte::<u32>(),
            I32GeS => self.run_gte::<i32>(),
            I32GeU => self.run_gte::<u32>(),

            I64Eqz => self.run_eqz::<i64>(),
            I64Eq => self.run_eq::<i64>(),
            I64Ne => self.run_ne::<i64>(),
            I64LtS => self.run_lt::<i64>(),
            I64LtU => self.run_lt::<u64>(),
            I64GtS => self.run_gt::<i64>(),
            I64GtU => self.run_gt::<u64>(),
            I64LeS => self.run_lte::<i64>(),
            I64LeU => self.run_lte::<u64>(),
            I64GeS => self.run_gte::<i64>(),
            I64GeU => self.run_gte::<u64>(),

            F32Eq => self.run_eq::<F32>(),
            F32Ne => self.run_ne::<F32>(),
            F32Lt => self.run_lt::<F32>(),
            F32Gt => self.run_gt::<F32>(),
            F32Le => self.run_lte::<F32>(),
            F32Ge => self.run_gte::<F32>(),

            F64Eq => self.run_eq::<F64>(),
            F64Ne => self.run_ne::<F64>(),
            F64Lt => self.run_lt::<F64>(),
            F64Gt => self.run_gt::<F64>(),
            F64Le => self.run_lte::<F64>(),
            F64Ge => self.run_gte::<F64>(),

            I32Clz => self.run_clz::<i32>(),
            I32Ctz => self.run_ctz::<i32>(),
            I32Popcnt => self.run_popcnt::<i32>(),
            I32Add => self.run_add::<i32>(),
            I32Sub => self.run_sub::<i32>(),
            I32Mul => self.run_mul::<i32>(),
            I32DivS => self.run_div::<i32, i32>(),
            I32DivU => self.run_div::<i32, u32>(),
            I32RemS => self.run_rem::<i32, i32>(),
            I32RemU => self.run_rem::<i32, u32>(),
            I32And => self.run_and::<i32>(),
            I32Or => self.run_or::<i32>(),
            I32Xor => self.run_xor::<i32>(),
            I32Shl => self.run_shl::<i32>(0x1F),
            I32ShrS => self.run_shr::<i32, i32>(0x1F),
            I32ShrU => self.run_shr::<i32, u32>(0x1F),
            I32Rotl => self.run_rotl::<i32>(),
            I32Rotr => self.run_rotr::<i32>(),

            I64Clz => self.run_clz::<i64>(),
            I64Ctz => self.run_ctz::<i64>(),
            I64Popcnt => self.run_popcnt::<i64>(),
            I64Add => self.run_add::<i64>(),
            I64Sub => self.run_sub::<i64>(),
            I64Mul => self.run_mul::<i64>(),
            I64DivS => self.run_div::<i64, i64>(),
            I64DivU => self.run_div::<i64, u64>(),
            I64RemS => self.run_rem::<i64, i64>(),
            I64RemU => self.run_rem::<i64, u64>(),
            I64And => self.run_and::<i64>(),
            I64Or => self.run_or::<i64>(),
            I64Xor => self.run_xor::<i64>(),
            I64Shl => self.run_shl::<i64>(0x3F),
            I64ShrS => self.run_shr::<i64, i64>(0x3F),
            I64ShrU => self.run_shr::<i64, u64>(0x3F),
            I64Rotl => self.run_rotl::<i64>(),
            I64Rotr => self.run_rotr::<i64>(),

            F32Abs => self.run_abs::<F32>(),
            F32Neg => self.run_neg::<F32>(),
            F32Ceil => self.run_ceil::<F32>(),
            F32Floor => self.run_floor::<F32>(),
            F32Trunc => self.run_trunc::<F32>(),
            F32Nearest => self.run_nearest::<F32>(),
            F32Sqrt => self.run_sqrt::<F32>(),
            F32Add => self.run_add::<F32>(),
            F32Sub => self.run_sub::<F32>(),
            F32Mul => self.run_mul::<F32>(),
            F32Div => self.run_div::<F32, F32>(),
            F32Min => self.run_min::<F32>(),
            F32Max => self.run_max::<F32>(),
            F32Copysign => self.run_copysign::<F32>(),

            F64Abs => self.run_abs::<F64>(),
            F64Neg => self.run_neg::<F64>(),
            F64Ceil => self.run_ceil::<F64>(),
            F64Floor => self.run_floor::<F64>(),
            F64Trunc => self.run_trunc::<F64>(),
            F64Nearest => self.run_nearest::<F64>(),
            F64Sqrt => self.run_sqrt::<F64>(),
            F64Add => self.run_add::<F64>(),
            F64Sub => self.run_sub::<F64>(),
            F64Mul => self.run_mul::<F64>(),
            F64Div => self.run_div::<F64, F64>(),
            F64Min => self.run_min::<F64>(),
            F64Max => self.run_max::<F64>(),
            F64Copysign => self.run_copysign::<F64>(),

            I32WrapI64 => self.run_wrap::<i64, i32>(),
            I32TruncSF32 => self.run_trunc_to_int::<F32, i32, i32>(),
            I32TruncUF32 => self.run_trunc_to_int::<F32, u32, i32>(),
            I32TruncSF64 => self.run_trunc_to_int::<F64, i32, i32>(),
            I32TruncUF64 => self.run_trunc_to_int::<F64, u32, i32>(),
            I64ExtendSI32 => self.run_extend::<i32, i64, i64>(),
            I64ExtendUI32 => self.run_extend::<u32, u64, i64>(),
            I64TruncSF32 => self.run_trunc_to_int::<F32, i64, i64>(),
            I64TruncUF32 => self.run_trunc_to_int::<F32, u64, i64>(),
            I64TruncSF64 => self.run_trunc_to_int::<F64, i64, i64>(),
            I64TruncUF64 => self.run_trunc_to_int::<F64, u64, i64>(),
            F32ConvertSI32 => self.run_extend::<i32, F32, F32>(),
            F32ConvertUI32 => self.run_extend::<u32, F32, F32>(),
            F32ConvertSI64 => self.run_wrap::<i64, F32>(),
            F32ConvertUI64 => self.run_wrap::<u64, F32>(),
            F32DemoteF64 => self.run_wrap::<F64, F32>(),
            F64ConvertSI32 => self.run_extend::<i32, F64, F64>(),
            F64ConvertUI32 => self.run_extend::<u32, F64, F64>(),
            F64ConvertSI64 => self.run_extend::<i64, F64, F64>(),
            F64ConvertUI64 => self.run_extend::<u64, F64, F64>(),
            F64PromoteF32 => self.run_extend::<F32, F64, F64>(),
    ...
    {
        let stack_value: T =
            <_>::from_runtime_value_internal(self.value_stack.pop());
        ...
        T: PartialEq<T> + Default,
    {
        ...
        let v =
            RuntimeValueInternal(if v == Default::default() { 1 } else { 0 });
        self.value_stack.push(v)?;
        Ok(InstructionOutcome::RunNextInstruction)
    }
